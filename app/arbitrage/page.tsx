...
import { opportunities, stats } from "@/data/arbitrage";

// Mock feature access hook - replace with actual implementation
const useFeatureAccess = () => {
  return {
    hasArbitrage: false, // Set to true to show full functionality
    tier: "free", // 'free', 'pro', or 'elite'
  };
};

export default function ArbitragePage() {
  const { hasArbitrage, tier } = useFeatureAccess();
    ...
    <div className="min-h-screen bg-background">
      ...
      >
        <div className="flex items-center gap-3">
          ...
          {tier === "elite" && <Badge className="bg-primary">Elite</Badge>}
        </div>
        <div className="flex items-center gap-2 text-small text-muted-foreground">
          <ArrowsCounterClockwise weight="regular" className="h-4 w-4" />
          ...
          <Card className="border-primary/50 bg-primary/5">
            <CardContent className="p-4 md:p-6 flex flex-col md:flex-row items-center gap-4">
              ...
              <div className="flex-1 text-center md:text-left">
                <h3 className="text-subtitle font-semibold mb-1">Upgrade to Professional</h3>
                <p className="text-small text-muted-foreground">
                  Real-time arbitrage scanning, cross-platform detection, and instant alerts when
                          ...
                          >
                            <span
                              className={`w-1.5 h-1.5 rounded-full mr-1.5 ${
                                opp.status === "active" ? "bg-success" : "bg-warning"
                              }`}
                            />
                            {opp.status === "active" ? "Active" : "Fading"}
                          </Badge>
                          <span className="text-caption text-muted-foreground flex items-center gap-1">
                            <Clock weight="regular" className="h-3 w-3" /> {opp.timeDetected}
                          </span>
                        </div>
                        <h3 className="text-small md:text-body font-medium truncate">
                          {opp.market}
                        </h3>
                            ...
                            <div className="grid sm:grid-cols-2 gap-4 mb-4">
                              <div className="p-3 rounded-md bg-secondary/50">
                                ...
                                <div className="space-y-1 text-caption">
                                  <div>
                                    Buy YES on {opp.platform2}: $
                                    {((opp.odds2 / 100) * opp.capital).toFixed(0)}
                                  </div>
                                  <div>
                                    Sell YES on {opp.platform1}: $
                                    {(((100 - opp.odds1) / 100) * opp.capital).toFixed(0)}
                                  </div>
                                </div>
                              </div>
                              <div className="p-3 rounded-md bg-secondary/50">
                                <div className="text-small font-medium mb-2">Expected Outcome</div>
                                <div className="space-y-1 text-caption">
                                  <div>Total Capital: ${opp.capital.toLocaleString()}</div>
                                  <div className="text-success font-medium">
                                    Guaranteed Profit: $
                                    {(opp.expectedReturn - opp.capital).toFixed(0)} ({opp.profit}%)
                                  </div>
                                </div>
                              </div>
                            </div>
        ...
        >
          <Card>
            <CardHeader className="p-3 md:p-4 pb-2">
              <CardTitle className="text-body md:text-subtitle">Today&apos;s Stats</CardTitle>
            </CardHeader>
            <CardContent className="p-3 md:p-4 pt-0 space-y-4">
              {stats.map((stat) => (
                <div key={stat.label} className="flex items-center justify-between">
                  ...
                  <span
                    className={`text-small font-semibold ${!hasArbitrage ? "blur-sm" : ""}`}
                  >
                    {stat.value}
                  </span>
                </div>
